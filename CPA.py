...
for ds in range(5): # for five big_dataset
    
    path = f'ProblemSet4/big_dataset_{ds}.npz'
    data = np.load(path, allow_pickle=True)
    t = data['t']   # trace[1280][5000]
    #print(np.shape(t))
    p = data['p']   # plaintext[1280][16]
    #print(p[1])
    c = data['c']   # ciphertext[1280][16]


    t_bar = np.sum(t, axis=0)/len(t)
    o_t = np.sqrt(np.sum((t - t_bar)**2, axis=0))
    ...
    for bnum in trange(0, 16):
        maxcpa = [0] * 256
        for kguess in range(0, 256):    # for each key guess

            hws = np.array([[HW[aes_internal(textin[bnum],kguess)] for textin in p]]).transpose()
            hws_bar = mean(hws)
            o_hws = std_dev(hws, hws_bar)
            correlation = cov(t, t_bar, hws, hws_bar)
            cpaoutput = correlation/(o_t*o_hws)
            maxcpa[kguess] = max(abs(cpaoutput))
        bestguess[bnum] = np.argmax(maxcpa) # best guess
